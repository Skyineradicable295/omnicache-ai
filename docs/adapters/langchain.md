...
Drop-in LLM response cache for LangChain. `LangChainCacheAdapter` implements `langchain_core.caches.BaseCache`, so you can register it globally with `set_llm_cache()` and every LLM call in your application is automatically cached through OmniCache-AI.

## Overview

LangChain supports a global LLM cache that intercepts all calls to chat models and LLMs. By providing an adapter that subclasses `BaseCache`, OmniCache-AI integrates seamlessly: LangChain calls `lookup()` before every LLM invocation and `update()` after, and the adapter maps these to `CacheManager.get()` and `CacheManager.set()`.
...
- You want to use OmniCache-AI backends (in-memory, disk, Redis) as the storage layer for LangChain's cache.
...
This installs `langchain-core >= 0.2` alongside `omnicache-ai`.
